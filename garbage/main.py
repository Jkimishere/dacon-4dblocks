...
class Loader(data.Dataset):
    def __init__(self, transforms, validation=False) -> None:
        ...
        rand = self.csv.query('A == 1').sample(n = 3932)
        self.df = pd.concat([self.csv, rand])
...
img_set = Loader(transforms=T.Compose([T.Resize((100,100)), T.RandomHorizontalFlip(), T.ToTensor(), T.Normalize((0.7854), (0.4284))]))
